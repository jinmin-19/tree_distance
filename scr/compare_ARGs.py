import sys
import os
import argparse
import time
from treeswift import *
import treeswift
import json
import numpy as np
import tskit
import random

import multiple_tree_matching

def normalize_tree(tree):
	height = tree.height()
	for n in tree.traverse_preorder():
		if n.edge_length:
			n.edge_length /= height

def __label_tree__(tree_obj):
	is_labeled = True
	i = 0
	labels = set()
	for node in tree_obj.traverse_preorder():
		if not node.is_root():
			if not node.edge_length:
				node.edge_length = 0
		if node.is_leaf():
			continue
		if not node.label or node.label in labels or node.label[0] != 'I': 
			is_labeled = False
			node.label = 'I' + str(i)
			i += 1        
		labels.add(node.label)
	return is_labeled


def hmean(vals):
	y = [1/x for x in vals]
	return len(y) / np.sum(y)

def log_mean(vals):
	return np.exp(np.mean(np.log(vals)))

def compare_trees_pairwise_distances(tree1, tree2):
	# print(tree1, tree2)
	tree1_obj = read_tree_newick(tree1)
	tree2_obj = read_tree_newick(tree2)

	normalize_tree(tree1_obj)
	normalize_tree(tree2_obj)

	# print(tree1_obj.newick(), tree2_obj.newick())

	tree1_size = len([n.label for n in tree1_obj.traverse_leaves()])
	tree2_size = len([n.label for n in tree2_obj.traverse_leaves()])

	if tree1_size != tree2_size:
		print("Trees not the same size")
		return

	leaves = [n.label for n in tree2_obj.traverse_leaves()]

	dist_1 = tree1_obj.distance_matrix(leaf_labels=True)
	dist_2 = tree2_obj.distance_matrix(leaf_labels=True)

	MSE = 0
	for i in range(len(leaves)):
		for j in range(i+1,len(leaves)):
			l1 = leaves[i]
			l2 = leaves[j]
			MSE += (dist_1[l1][l2] - dist_2[l1][l2])**2

	total = len(leaves) * (len(leaves) - 1)/2
	return MSE/total

def compare_trees_TCMM(tree1, tree2, method = 'mean'):
	tree1_obj = read_tree_newick(tree1)
	tree2_obj = read_tree_newick(tree2)

	normalize_tree(tree1_obj)
	normalize_tree(tree2_obj)

	__label_tree__(tree1_obj)
	__label_tree__(tree2_obj)

	leaves = [n.label for n in tree1_obj.traverse_leaves()]

	new_tree, obj, rates, bls = multiple_tree_matching.compute_optimal_rates(tree1_obj, tree2_obj, r = 0)

	# print(obj)
	rates = [float(r) for r in rates]

	return [np.mean(rates) - 1 + np.std(rates), hmean(rates) - 1 + np.std(rates), log_mean(rates) + np.std(rates),rates]

	if method == 'mean':
		return np.mean(rates) - 1 + np.std(rates)
	if method == 'hmean':
		return hmean(rates) - 1 + np.std(rates)
	if method == 'log_mean':
		return log_mean(rates) + np.std(rates)
	# print(norm_rates)
	# print(np.mean(norm_rates), np.std(norm_rates))
	# return obj
	# dist = log_mean(norm_rates) + np.std(norm_rates)
	# dist = np.mean(norm_rates) + np.std(norm_rates)
	return dist



def main():
	parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
	parser.add_argument('-t', '--true', required=True, help="True ARG")
	parser.add_argument('-i', '--inferred', required=True, help="Inferred ARG")
	parser.add_argument('-o', '--output', required=True, help="output file")
	# parser.add_argument('-e', '--est_queries', required=True, help="Estimated queries")
	parser.add_argument('-m', '--method', required=False, choices=['mean', 'hmean', 'log_mean', 'kc', 'pairwise'], default='mean', help="Summarization method")

	args = parser.parse_args()

	true_arg=tskit.load(args.true)
	inferred_arg=tskit.load(args.inferred)

	if true_arg.sequence_length != inferred_arg.sequence_length:
		print("Two args do not have the same length")
		return

	if args.method == 'kc':
		samples = true_arg.samples()
		true_arg = true_arg.simplify(samples=samples)
		inferred_arg = inferred_arg.simplify(samples=samples)

	seq_length = true_arg.sequence_length
	n = 1000

	all_dists = []
	all_random = []

	for i in range(n):
		pos = int(seq_length / n * i)
		t_tree = true_arg.at(pos, sample_lists=True)
		i_tree = inferred_arg.at(pos, sample_lists=True)

		if (t_tree.num_roots>1 or i_tree.num_roots>1):
			print("trees have multiple roots")
			continue
		if args.method == 'kc':
			dist = t_tree.kc_distance(i_tree)
			all_dists.append([pos, 'kc', dist])
		elif args.method == 'pairwise':
			dist = compare_trees_pairwise_distances(t_tree.as_newick(), i_tree.as_newick())
			all_dists.append([pos, 'pairwise', dist])
		else:
			dist = compare_trees_TCMM(t_tree.as_newick(), i_tree.as_newick(), args.method)
			all_dists.append([pos, 'mean', dist[0]])
			all_dists.append([pos, 'hmean', dist[1]])
			all_dists.append([pos, 'log_mean', dist[2]])

		random_pos = random.randint(1, seq_length)
		r_tree = inferred_arg.at(random_pos, sample_lists=True)

		if (t_tree.num_roots>1 or r_tree.num_roots>1):
			print("trees have multiple roots")
			continue

		if args.method == 'kc':
			random_dist = t_tree.kc_distance(r_tree)
			all_random.append([pos, random_pos, 'kc', random_dist])
		elif args.method == 'pairwise':
			random_dist = compare_trees_pairwise_distances(t_tree.as_newick(), r_tree.as_newick())
			all_random.append([pos, random_pos, 'pairwise', random_dist])
		else:
			random_dist = compare_trees_TCMM(t_tree.as_newick(), r_tree.as_newick(), args.method)
			all_random.append([pos, random_pos, 'mean', random_dist[0]])
			all_random.append([pos, random_pos, 'hmean', random_dist[1]])
			all_random.append([pos, random_pos, 'log_mean', random_dist[2]])
		# if args.method == 'kc':
		# 	random_dist = t_tree.kc_distance(r_tree)
		# else:
		# 	random_dist = compare_trees_TCMM(t_tree.as_newick(), r_tree.as_newick(), args.method)
		print(dist, random_dist)
		# all_random.append([pos, random_pos, random_dist])


	with open(args.output,'w') as f:
		for d in all_dists:
			f.write(str(d[0]) + "\t" + str(d[0]) + "\t" + str(d[1]) + "\t" + str(d[2]) + "\n")
		for d in all_random:
			f.write(str(d[0]) + "\t" + str(d[1]) + "\t" + str(d[2]) + "\t" + str(d[3]) + "\n")


if __name__ == "__main__":
	main()